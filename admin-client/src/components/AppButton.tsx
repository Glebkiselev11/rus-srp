...
export function AppButton({
  ...
  const buttonClasses = useMemo(() => {
    const inlineBgClasses = `bg-transparent ${
      !disabled && "hover:bg-indigo-100"
    }`;
    const filledBgClasses = `bg-indigo-600 ${
      disabled ? "bg-zinc-900/[.12]" : "hover:bg-indigo-600/[.92]"
    }`;
  ...
  return (
    ...
    >
      {icon && <AppIcon name={icon} color={iconColor} size={iconSize} />}

      {icon && label && <span className="w-[8px]" />}

      {label && <span>{loading ? t("wait") : label}</span>}
    </button>
  );
}
